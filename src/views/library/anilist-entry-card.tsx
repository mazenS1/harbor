...
export function AnilistEntryCard({
  ...
  const m = entry.media;
  const name = m.title.userPreferred || m.title.english || m.title.romaji || "Untitled";
  ...
  return (
    <div className="group relative flex flex-col gap-2 text-left">
      ...
      >
        ...
        <span
          role="button"
          aria-label="Remove from AniList"
          onClick={(e) => {
            e.stopPropagation();
            if (!busy) onRemove();
          }}
          className="absolute right-2 top-2 flex h-8 w-8 items-center justify-center rounded-full bg-canvas/85 text-ink-muted opacity-0 ring-1 ring-edge-soft backdrop-blur-sm transition-opacity hover:text-danger group-hover:opacity-100"
        >
          <Trash2 size={13} />
        </span>
      </button>

      <button type="button" onClick={open} className="truncate text-left text-[13px] font-medium text-ink">
        {name}
      </button>
      ...
      <div className="flex items-center justify-between rounded-lg bg-elevated/50 ring-1 ring-edge-soft">
        <button
          ...
          className="flex h-11 w-11 items-center justify-center rounded-l-lg text-ink-muted transition-colors hover:bg-raised hover:text-ink disabled:opacity-40"
        >
        ...
        <button
          ...
          onClick={() => onProgress(entry.progress + 1)}
          className="flex h-11 w-11 items-center justify-center rounded-r-lg text-ink-muted transition-colors hover:bg-raised hover:text-ink disabled:opacity-40"
        >
